<script>
	...
	function removeItem(value){
		...
		passwords = passwords.filter(pass=>pass!==value)
	}

	$:tooShort = password!="" && password.length<5
...
<section>
	...
	{/if}
	{#if tooLong}
		<h3>Password is too long</h3>
	{/if}
</section>
<section>
	<ul>
	{#each passwords as pass,index (pass)}
		<li on:click={()=>{removeItem(pass)}}>{pass}</li>
	{/each}
	</ul>
</section>